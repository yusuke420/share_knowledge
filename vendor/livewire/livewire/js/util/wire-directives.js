...
class Directive {
    ...
    durationOr(defaultDuration) {
        ...
        const durationInMilliSecondsString = this.modifiers.find((mod) =>
            mod.match(/([0-9]+)ms/)
        );
        const durationInSecondsString = this.modifiers.find((mod) =>
            mod.match(/([0-9]+)s/)
        );
        ...
        if (durationInMilliSecondsString) {
            ...
            durationInMilliSeconds =
                Number(durationInSecondsString.replace("s", "")) * 1000;
        }
