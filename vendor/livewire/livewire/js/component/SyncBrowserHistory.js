...
export default function () {
    ...
    store.registerHook("component.initialized", (component) => {
        ...
        setTimeout(() => {
            let url = onlyChangeThePathAndQueryString(
                initializedPath ? undefined : component.effects.path
            );
    ...
    function onlyChangeThePathAndQueryString(url) {
        ...
        let afterOrigin = destination.href
...
class LivewireState {
    ...
    pushItemInProperOrder(signature, response, component) {
        ...
        let closestParentId = store.getClosestParentId(
            component.id,
    ...
    findComponentBySignature(signature) {
        ...
        return (
            ...
            console.warn(
                `Livewire: couldn't find component on page: ${componentName}`
            )
        );
    }

    parseSignature(signature) {
        let [originalComponentId, componentName, componentIndex] =
            signature.split(":");
