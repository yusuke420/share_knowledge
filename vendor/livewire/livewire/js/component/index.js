...
export default class Component {
    ...
    checkForMultipleRootElements() {
        ...
        let countElementsBeforeMarker = (el, carryCount = 0) => {
            ...
            if (
                ...
                el.textContent.includes(`wire-end:${this.id}`)
            )
                return carryCount;
    ...
    get(name) {
        ...
        return name
            ...
            .reduce(
                (carry, segment) =>
                    typeof carry === "undefined" ? carry : carry[segment],
                this.data
            );
    }
    ...
    addAction(action) {
        ...
        if (
            ...
            const message =
                this.prefetchManager.getPrefetchMessageByAction(action);
    ...
    handleMorph(dom) {
        ...
        morphdom(this.el, dom, {
            ...
            onNodeAdded: (node) => {
                const closestComponentId =
                    DOM.closestRoot(node).getAttribute("wire:id");
                ...
                if (typeof property === "string" && property.match(/^emit.*/))
                    return function (...args) {
                        if (property === "emitSelf")
                            return store.emitSelf(component.id, ...args);
                        if (property === "emitUp")
                            return store.emitUp(component.el, ...args);
